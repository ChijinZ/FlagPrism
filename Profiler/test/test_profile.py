"""
Reproducibility tests for FlagTree Profiler.
...
import flagtree.profiler.hooks.launch as profiler_launch
...
def test_hook_with_third_party(tmp_path: pathlib.Path):
    ...
    profiler_hook_invoked = False

    def metadata_fn(grid: tuple, metadata: NamedTuple, args: dict):
        nonlocal profiler_hook_invoked
        profiler_hook_invoked = True
...
def test_hook_multiple_threads(tmp_path: pathlib.Path):
    ...
    def invoke_foo():
        for _ in range(100):
            ...
            all_ids.add(profiler_launch.id.get())
...
def test_pcsampling(tmp_path: pathlib.Path):
    ...
    if os.environ.get("FLAGTREE_PROFILER_SKIP_PC_SAMPLING_TEST", "0") == "1":
