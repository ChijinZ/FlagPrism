"""
Minimal smoke test for FlagTree Profiler's CANN vendor backend.

Run on the server with:

    python -m pytest -q third_party/FlagPrism/Profiler/test/test_cann_smoke.py -s
...
def _use_legacy_cann_triton_hook(monkeypatch):
    monkeypatch.setenv("FLAGTREE_PROFILER_CANN_TRITON_HOOK_LEGACY", "1")


def _finalize_cann_import(base, import_root, monkeypatch, metrics="aicore"):
    monkeypatch.setenv("FLAGTREE_PROFILER_CANN_IMPORT_PATH", str(import_root))
...
def test_ir_record_buffer_capacity(monkeypatch):
    monkeypatch.delenv("FLAGTREE_PROFILER_IR_RECORD_BUFFER_MB", raising=False)
    assert profiler_profile._instrumentation_record_capacity() == 524288

    monkeypatch.setenv("FLAGTREE_PROFILER_IR_RECORD_BUFFER_MB", "64")
    assert profiler_profile._instrumentation_record_capacity() == 1048576

    monkeypatch.setenv("FLAGTREE_PROFILER_IR_RECORD_BUFFER_MB", "0")
    with pytest.raises(ValueError, match="must be a positive integer"):
        profiler_profile._instrumentation_record_capacity()
...
def test_ir_record_capacity_is_part_of_cache_mode(monkeypatch):
    ...
    monkeypatch.setenv("FLAGTREE_PROFILER_IR_RECORD_BUFFER_MB", "32")
    profiler_profile._activate_instrumentation()
    try:
...
def test_native_instrumentation_preserves_mode_and_default_hook(monkeypatch):
    ...
    runtime = SimpleNamespace(start=start)
    ...
    monkeypatch.setattr(
        profiler_profile,
        "InstrumentationHook",
        lambda selected_mode: ("instrumentation", selected_mode),
    )
    monkeypatch.setattr(
    ...
    try:
        session = profiler_profile.start(
            "/tmp/flagtree_profiler_instrumentation_compat",
            backend="instrumentation",
            mode=mode,
        )
        assert session == 71
        assert calls["start"][4] == f"cuda:{mode}"
        assert calls["hook"] == (("instrumentation", mode), 71)
    finally:
        profiler_profile._active_sessions.clear()
...
def test_finalize_preserves_data_specific_default_format(monkeypatch):
    ...
    monkeypatch.setattr(
        profiler_profile.HookManager,
        "unregister",
        staticmethod(lambda session=None: None),
    )
    profiler_profile._active_sessions.clear()
    profiler_profile._active_sessions[72] = {"name": "trace"}
    profiler_profile.finalize(72)
...
def real_cann_direct_run(tmp_path_factory):
    ...
    cmd = [
        ...
        "--iters",
        env.get("FLAGTREE_PROFILER_CANN_TEST_ITERS", "3"),
        "--warmup",
        env.get("FLAGTREE_PROFILER_CANN_TEST_WARMUP", "1"),
    ]
    result = subprocess.run(
        ...
        timeout=int(env.get("FLAGTREE_PROFILER_CANN_TEST_TIMEOUT", "240")),
    )
...
def test_cann_backend_smoke(tmp_path):
    ...
    vendor_output = tmp_path / "flagtree_profiler_cann_profile"
    ...
    profiler_native.enter_op(scope_id, "cann_smoke_kernel")
    time.sleep(0.001)
    profiler_native.exit_op(scope_id, "cann_smoke_kernel")
...
def test_cann_hatchet_preserves_custom_scope_data(tmp_path):
    base = tmp_path / "profile_scope_data"
    session_id = profiler.start(
        name=str(base),
        context="python",
        data="tree",
    ...
    while pending:
        node = pending.pop()
        if node["metrics"].get("custom_metric") == 1.5:
            custom_scope = node
            break
        pending.extend(node["children"])

    assert custom_scope is not None
    assert custom_scope["metrics"]["custom_metric"] == 1.5
    assert custom_scope["metrics"]["custom_property"] == "kept"
